...
qui replace region="ÖÐ²¿" if inlist(id,4,7,8,12,14,16,17,18)
...
label var edu "¸ßÖÐÑ§ÀúÒÔÉÏÕ¼×ÜÈË¿Ú±È£¨%£©"
...
label var fd1 "µØ·½Ô¤ËãÄÚ²ÆÕþ×ÔÖ÷¶È"
label var PT "¹ãÒåºê¹ÛË°¸º(%)"
label var plandlea_sq "ÈË¾ùÍÁµØ³öÈÃÃæ»ý(¹«Çê/ÈË)"
...
, ///
 ...
 ytitle("ÈË¾ùÍÁµØ³öÈÃ½ð(Ôª)") ///
  ylabel(0(4000)8000) ymtick(##1) ///
  xlabel(0(20000)80000) xmtick(##3) ///
 legend(label(1 "É¢µã") label(2 "ÄâºÏ") size(*0.7)) ///
 note("×ÊÁÏÀ´Ô´£º1999-2012Äê¡¶ÖÐ¹ú¹úÍÁ×ÊÔ´Äê¼ø¡·ºÍ¡¶ÖÐ¹úÍ³¼ÆÄê¼ø¡·",size(small)) ///
 caption("ËµÃ÷£ºÊ¡¼¶Êý¾Ý",size(small)) scheme(s1mono) ///
...
,  ///
 ...
 xlabel(0(20000)80000) xmtick(##3) ///
 legend(label(1 "É¢µã") label(2 "ÄâºÏ") size(*0.7)) ///
 note("×ÊÁÏÀ´Ô´£º2004-2009Äê¡¶ÖÐ¹ú¹úÍÁ×ÊÔ´Äê¼ø¡·ºÍ¡¶ÖÐ¹úÍ³¼ÆÄê¼ø¡·",size(small)) ///
 caption("ËµÃ÷£ºÊ¡¼¶Êý¾Ý",size(small)) scheme(s1mono) ///
 ...
 legend(label(1 "É¢µã") label(2 "ÄâºÏ") size(*0.7)) ///
 note("×ÊÁÏÀ´Ô´£º1999-2012Äê¡¶ÖÐ¹ú¹úÍÁ×ÊÔ´Äê¼ø¡·ºÍ¸÷µØÇøÀúÄêÍ³¼ÆÄê¼ø",size(small)) ///
 caption("ËµÃ÷£ºÊ¡¼¶Êý¾Ý",size(small)) scheme(s1mono) ///
...
,  ///
 ...
 legend(label(1 "É¢µã") label(2 "ÄâºÏ") size(*0.7)) ///
 note("×ÊÁÏÀ´Ô´£º2004-2009Äê¡¶ÖÐ¹ú¹úÍÁ×ÊÔ´Äê¼ø¡·ºÍ¸÷µØÇøÀúÄêÍ³¼ÆÄê¼ø",size(small)) ///
 caption("ËµÃ÷£ºÊ¡¼¶Êý¾Ý",size(small)) scheme(s1mono) ///
...
outreg2 [gfe1 gfe2 gfe3 gfe4 gre1 gre2 gre3 gre4] using result1, word replace ///
 title("±í1:ÍÁµØ¶Ô¾­¼ÃÔö³¤µÄ¾²Ì¬Ð§Ó¦" )  /// // (T1)
 tdec(2) rdec(3) r2 e(F) /// // (T3)
 nonote /// // (T4)
 addnote("×¢£º(1)***,**,*·Ö±ð±íÊ¾ÔÚ1%,5%ºÍ10%Ë®Æ½ÉÏÏÔÖø;", ///
 "(2)À¨ºÅÖÐÎª±ê×¼Îó;", ///
 "(3)ÆäËü×¢ÊÍÓï¾ä¡£")
...
outreg2 [pfe1 pfe2 pfe3 pfe4 pre1 pre2 pre3 pre4] using result2, word replace ///
 title("±í2:ÍÁµØ¶ÔË½ÈËÍ¶×ÊµÄ¾²Ì¬Ð§Ó¦" )  /// // (T1)
 tdec(2) rdec(3) r2 e(F) /// // (T3)
 nonote /// // (T4)
 addnote("×¢£º(1)***,**,*·Ö±ð±íÊ¾ÔÚ1%,5%ºÍ10%Ë®Æ½ÉÏÏÔÖø;", ///
 "(2)À¨ºÅÖÐÎª±ê×¼Îó;", ///
...
local mm "fd_1s fd_2s sys_GMM1 sys_GMM2 sys_GMM3 OLS FE"
 ...
 title("±í3:ÖÐ¹ú¾­¼ÃÔö³¤ÓëÍÁµØ²ÆÕþµÄ¶¯Ì¬Ãæ°åFDÓësys-GMM¹À¼Æ1" )  /// // (T1)
 tdec(2) rdec(3) r2 e(F)   /// // (T3)
 nonote /// // (T4)
 addnote("×¢£º(1)***,**,*·Ö±ð±íÊ¾ÔÚ1%,5%ºÍ10%Ë®Æ½ÉÏÏÔÖø;", ///
 "(2)À¨ºÅÖÐÎª±ê×¼Îó;", ///
